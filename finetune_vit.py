...
from ssp_vit import ViTBackbone
...
def evaluate_test_data(model, test_loader, classes):
    ...
    plt.savefig("./ssp_retinaloct_midl2025/vit2net/result/confusion_matrix.png")
    plt.show()
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(small_labeled_dataset)), 
    ...
    model.backbone.load_state_dict(torch.load("./ssp_retinaloct_midl2025/vit2net/octmnist_vit2net_model.pth"))
...
plt.grid()
plt.savefig("./ssp_retinaloct_midl2025/vit2net/result/roc_curve_all_folds.png")
plt.show()