...
def evaluate_test_data(model, test_loader, classes):
    ...
    plt.savefig("./ssp_retinaloct_tbme/vit2spn_tiny/dsn_ssn/result/octird_dual_confusion_matrix.png")
    plt.show()
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(subset_dataset)), 
    ...
    train_targets = np.array([subset_dataset.dataset.targets[i] for i in train_idx]).squeeze()
...
plt.grid()
plt.savefig("./ssp_retinaloct_tbme/vit2spn_tiny/dsn_ssn/result/octird_dual_roc_curve_all_folds.png")
plt.show()