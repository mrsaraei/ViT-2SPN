...
def evaluate_test_data(model, test_loader, classes):
    ...
    plt.savefig("./ssp_retinaloct_tbme/vit2spn_tiny/dsn_ssn/result/octird_confusion_matrix.png")
    plt.show()
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(small_labeled_dataset)), 
    ...
    model.backbone.load_state_dict(torch.load("./ssp_retinaloct_tbme/vit2spn_tiny/dsn_ssn/octmnist_vitspn_tiny_model.pth"))
...
plt.grid()
plt.savefig("./ssp_retinaloct_tbme/vit2spn_tiny/dsn_ssn/result/octird_roc_curve_all_folds.png")
plt.show()