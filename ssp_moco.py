...
output_directory = "./ssp_retinaloct_midl2025/moco"
output_pretrained_model_path = os.path.join(output_directory, "octmnist_moco_model.pth")
...
class MoCoNetwork(nn.Module):
    def __init__(self):
        super().__init__()
        self.online_network = MoCoBackbone()
        self.target_network = MoCoBackbone()
...
# Cosine similarity loss for MoCo
...
model = MoCoNetwork().to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
criterion = MoCoLoss()
scaler = GradScaler()

# Training Function with Loss Logging
def train_moco(model, dataloader, epochs, optimizer, criterion):
    model.train()
    ...
    plt.grid(True)
    plt.savefig('./ssp_retinaloct_midl2025/moco/result/pretraining_loss_curv.png')
    plt.show()

# Start Training
print("Starting MoCo pretraining...")
train_moco(model, unlabeled_dataloader, epochs, optimizer, criterion)
