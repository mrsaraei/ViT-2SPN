...
output_directory = "./ssp_retinaloct_midl2025/simclrv2"
output_pretrained_model_path = os.path.join(output_directory, "octmnist_simclrv2_model.pth")
...
# Data Augmentation for SimCLRv2
...
# Stronger augmentation pipeline as used in SimCLR v2
strong_augment_transform = transforms.Compose([
    ...
    transforms.GaussianBlur(3, sigma=(0.1, 2.0)),  # Gaussian blur as in SimCLR v2
    transforms.RandomApply([transforms.Lambda(lambda x: transforms.functional.solarize(x, threshold=128))], p=0.2),
    transforms.ToTensor(),
...
class ResNetBackbone(nn.Module):
    def __init__(self):
        super().__init__()
        resnet = resnet50(pretrained=True)  # Use ResNet-50 as the base
        resnet.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)  # Change input channel to 1
        self.features = nn.Sequential(*list(resnet.children())[:-2])  # Remove final fully connected layer
...
# SimCLR v2 Model with ResNet Backbone
...
def train_self_supervised(model, dataloader, epochs, optimizer, criterion):
    ...
    plt.grid(True)
    plt.savefig('./ssp_retinaloct_midl2025/simclrv2/result/pretraining_loss_curv.png')
    plt.show()

# Start Training
print("Starting self-supervised pretraining with SimCLRv2...")
