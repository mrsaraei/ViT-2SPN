...
from ssp_simclrv2 import ResNetBackbone
...
def evaluate_test_data(model, test_loader, classes):
    ...
    plt.savefig("./ssp_retinaloct_midl2025/simclrv2/result/confusion_matrix.png")
    plt.show()
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(small_labeled_dataset)), 
    ...
    load_pretrained_backbone(model, "./ssp_retinaloct_midl2025/simclrv2/octmnist_simclrv2_model.pth")
    ...
    fpr_dict[fold], tpr_dict[fold], auc_dict[fold], mean_auc, _, _ = compute_auc_and_plot_fold(
...
plt.grid()
plt.savefig("./ssp_retinaloct_midl2025/simclrv2/result/roc_curve_all_folds.png")
plt.show()