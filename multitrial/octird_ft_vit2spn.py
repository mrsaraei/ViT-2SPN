...
def evaluate_test_data(model, loader, classes):
    ...
    return top1_accuracy, np.mean(sensitivities), np.mean(specificities), np.mean(probs.max(axis=1))
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(subset_dataset)), 
    ...
    train_labels = np.array([full_dataset.targets[i] for i in train_idx]).squeeze()
    ...
    fpr_dict[fold], tpr_dict[fold], auc_dict[fold], mean_auc, _, probs = compute_auc_and_plot_fold(model, val_loader, [str(i) for i in range(num_classes)], fold)
    fold_aucs.append(mean_auc)

    top1_accuracy, sensitivity, specificity, avg_confidence = evaluate_test_data(model, test_loader, [str(i) for i in range(num_classes)])
...
mean_auc_all_folds = np.mean(aucs)
std_auc_all_folds = np.std(aucs)

mean_top1_accuracy = np.mean(top1_accuracies)
std_top1_accuracy = np.std(top1_accuracies)
...
# Print results
print(f"\nMean Confidence Score: {mean_confidence:.4f} ± {std_confidence:.4f}")
print(f"Mean AUC across folds: {mean_auc_all_folds:.4f} ± {std_auc_all_folds:.4f}")
print(f"Mean Top-1 Accuracy: {mean_top1_accuracy:.4f} ± {std_top1_accuracy:.4f}")
