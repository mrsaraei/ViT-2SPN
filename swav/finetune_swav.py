...
from ssp_swav import ResNetBackbone
...
def compute_auc_and_plot_fold(model, val_loader, classes, fold):
    ...
    #plt.grid()
    #plt.savefig(f'./ssp_retinaloct_midl2025/swav/result/roc_curve_fold_{fold}.png')
    #plt.close()
...
def evaluate_test_data(model, test_loader, classes):
    ...
    plt.savefig("./ssp_retinaloct_midl2025/swav/result/confusion_matrix.png")
    plt.show()
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(small_labeled_dataset)), 
    ...
    model.backbone.load_state_dict(torch.load("./ssp_retinaloct_midl2025/swav/octmnist_swav_model.pth"))
...
plt.legend(loc="lower right")
plt.grid()
plt.savefig("./ssp_retinaloct_midl2025/swav/result/roc_curve_all_folds.png")
plt.show()