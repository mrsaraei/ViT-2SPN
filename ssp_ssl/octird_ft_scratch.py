...
# Debugging: Print indices and dataset sizes
print(f"Subset dataset size: {len(subset_dataset)}")
print(f"Train dataset size: {len(train_dataset)}")
print(f"Validation dataset size: {len(val_dataset)}")
print(f"Test dataset size: {len(test_dataset)}")

# Create a small labeled dataset based on the subset
small_labeled_indices = random.sample(range(len(subset_dataset)), min(len(subset_dataset), subset_size))
small_labeled_dataset = Subset(full_dataset, small_labeled_indices)
small_labeled_dataset.dataset.labels = [full_dataset.targets[i] for i in small_labeled_indices]

# Compute class sizes in each dataset
def compute_class_sizes(dataset):
    return Counter(full_dataset.targets[i] for i in dataset.indices)

# Get class sizes
train_class_sizes = compute_class_sizes(train_dataset)
val_class_sizes = compute_class_sizes(val_dataset)
test_class_sizes = compute_class_sizes(test_dataset)

# Print class sizes
print(f"Class sizes in training set: {train_class_sizes}")
print(f"Class sizes in validation set: {val_class_sizes}")
print(f"Class sizes in test set: {test_class_sizes}")
...
# Ensure the criterion uses the correct weight tensor size
...
def evaluate_test_data(model, test_loader, classes):
    ...
    plt.savefig("./ssp_retinaloct_tbme/vit2spn_tiny/scratch/result/octird_confusion_matrix.png")
    plt.show()
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(small_labeled_dataset)), 
    ...
    model.backbone.load_state_dict(torch.load("./ssp_retinaloct_tbme/vit2spn_tiny/scratch/octmnist_vit2spn_tiny_scratch_model.pth"))
...
plt.grid()
plt.savefig("./ssp_retinaloct_tbme/vit2spn_tiny/scratch/result/octird_roc_curve_all_folds.png")
plt.show()