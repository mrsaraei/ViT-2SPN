...
from ssp_byol import ResNetBackbone
...
def compute_auc_and_plot_fold(model, val_loader, classes, fold):
    ...
    # Store fpr, tpr, and auc for each fold in dictionaries
    fpr_dict[fold] = fpr
    tpr_dict[fold] = tpr
...
def evaluate_test_data(model, test_loader, classes):
    ...
    plt.savefig("./ssp_retinaloct_midl2025/byol/result/confusion_matrix.png")
    plt.show()
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(small_labeled_dataset)), 
    ...
    model.backbone.load_state_dict(torch.load("./ssp_retinaloct_midl2025/byol/octmnist_byol_model.pth"))
...
plt.grid()
plt.savefig("./ssp_retinaloct_midl2025/byol/result/roc_curve_all_folds.png")
plt.show()