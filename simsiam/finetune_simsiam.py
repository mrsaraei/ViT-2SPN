...
from ssp_simsiam import ResNetBackbone
...
print(f"Original dataset size: {len(labeled_dataset)}")
print(f"Subset size (for fine-tuning): {len(small_labeled_dataset)}")
print(f"Original test set size: {len(test_dataset)}")
print(f"Subset size (for testing): {len(test_subset)}")
...
class FineTunedModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.fc = nn.Sequential(
            nn.Linear(2048, 128),  # Adjusted input size to match ResNetBackbone output
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Dropout(0.5),
    ...
    def forward(self, x):
        ...
        print(f"Shape of features from ResNetBackbone: {features.shape}") 
...
def evaluate_test_data(model, test_loader, classes):
    ...
    plt.savefig("./ssp_retinaloct_midl2025/simsiam/result/confusion_matrix.png")
    plt.show()
...
for fold, (train_idx, val_idx) in enumerate(skf.split(range(len(small_labeled_dataset)), 
    ...
    model.backbone.load_state_dict(torch.load("./ssp_retinaloct_midl2025/simsiam/octmnist_simsiam_model.pth"))
    ...
    fpr, tpr, roc_auc, mean_auc, val_labels, val_probs = compute_auc_and_plot_fold(model, val_loader, list(range(num_classes)), fold)
...
evaluate_test_data(best_model, test_loader, list(range(num_classes)))
...
plt.grid()
plt.savefig("./ssp_retinaloct_midl2025/simsiam/result/roc_curve_all_folds.png")
plt.show()