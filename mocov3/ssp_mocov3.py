...
epochs = 50
learning_rate = 1e-4
momentum = 0.99  # MoCo momentum
accumulation_steps = 4  # Gradient accumulation steps
output_directory = "./ssp_retinaloct_midl2025/mocov3"
output_pretrained_model_path = os.path.join(output_directory, "octmnist_mocov3_model.pth")
...
# Data Augmentation for MoCov3 (using stronger augmentations)
...
# MoCoV3 Network (multi-crop setup)
class MoCoV3Network(nn.Module):
    def __init__(self):
        super().__init__()
        self.online_network = MoCoV3Backbone()
        self.target_network = MoCoV3Backbone()
...
# Cosine similarity loss for MoCoV3
...
model = MoCoV3Network().to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
criterion = MoCoV3Loss()
scaler = GradScaler()

# Training Function with Loss Logging
def train_moco_v3(model, dataloader, epochs, optimizer, criterion):
    model.train()
    ...
    plt.grid(True)
    plt.savefig('./ssp_retinaloct_midl2025/mocov3/result/pretraining_loss_curv.png')
    plt.show()

# Start Training
print("Starting MoCoV3 pretraining...")
train_moco_v3(model, unlabeled_dataloader, epochs, optimizer, criterion)
