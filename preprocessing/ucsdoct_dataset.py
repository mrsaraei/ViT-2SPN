...
# ========== Visualize Some Samples (with Grayscale Support) ==========
...
def visualize_samples(data_dir, classes, num_samples=5):
    ...
    for i, cls in enumerate(classes):
        ...
        for j, img_name in enumerate(images):
            ...
            try:
                ...
                # Convert and display grayscale or RGB image
                if img.mode != 'RGB':
                ...
                if j == 0:
                    ax.text(-0.1, 0.5, cls.upper(), color='black', fontsize=18,
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'ucsdoct_sample_images.png'))
    plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'ucsdoct_class_distribution.png'))
plt.show()
...
summary_path = os.path.join(output_dir, 'ucsdoct_dataset_summary.json')
